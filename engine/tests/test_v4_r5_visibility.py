...
def _counting_nop(obs: ObservationV2, state: dict[str, Any]) -> AgentAction:
    ...
    state.setdefault("visibility", []).append((obs.current_tick, obs.visible_enemy_anchor_addresses))
...
def _process(
    ...
    position: int,
    share: int,
    logic=_counting_nop) -> ProcessInstance:
...
def _searching_attacker(obs: ObservationV2, state: dict[str, Any]) -> AgentAction:
    ...
    if obs.visible_enemy_anchor_addresses:
        if "first_detection_tick" not in state:
            state["first_detection_tick"] = obs.current_tick
            state["first_detection_action"] = action_index
        target = min(
            obs.visible_enemy_anchor_addresses,
            key=lambda address: abs(_signed_delta(obs.self_anchor or 0, address, obs.arena_size)))
    ...
    delta = _signed_delta(obs.self_anchor or 0, target, obs.arena_size)
    if abs(delta) > (obs.self_reach or 0):
        state["pursuit_moves"] = state.get("pursuit_moves", 0) + 1
        return AgentAction(ActionKindV2.MOVE, max(-64, min(64, delta)))

    if state.get("last_attempt_tick") != obs.current_tick:
        state["last_attempt_tick"] = obs.current_tick
...
def _run_static_search(
    target_position: int) -> tuple[ProcessInstance, ProcessInstance]:
...
def test_detection_is_evaluated_before_each_callback_after_movement() -> None:
    def observer_logic(obs: ObservationV2, state: dict[str, Any]) -> AgentAction:
        state.setdefault("trace", []).append((obs.self_anchor, obs.visible_enemy_anchor_addresses))
        if not obs.visible_enemy_anchor_addresses:
            return AgentAction(ActionKindV2.MOVE, 10)
        return AgentAction(ActionKindV2.NOP)
...
def _run_moving_target(
    ...
    def scout_logic(obs: ObservationV2, state: dict[str, Any]) -> AgentAction:
        state.setdefault("trace", []).append(
            (obs.current_tick, obs.self_anchor, obs.visible_enemy_anchor_addresses)
        )
        if obs.visible_enemy_anchor_addresses:
            obs.shared_memory["last_known"] = obs.visible_enemy_anchor_addresses[0]
            obs.shared_memory["last_seen_tick"] = obs.current_tick
            return AgentAction(ActionKindV2.NOP)
        if obs.current_tick >= 2:
            state["search_moves"] = state.get("search_moves", 0) + 1
            return AgentAction(ActionKindV2.MOVE, 64)
        return AgentAction(ActionKindV2.NOP)
    ...
    def target_logic(obs: ObservationV2, state: dict[str, Any]) -> AgentAction:
        callback_by_tick = state.setdefault("callback_by_tick", {})
        callback_index = callback_by_tick.get(obs.current_tick, 0)
        callback_by_tick[obs.current_tick] = callback_index + 1
        if callback_index == 0:
            return AgentAction(ActionKindV2.MOVE, 64)
        return AgentAction(ActionKindV2.NOP)
...
def test_local_detection_selects_spatial_targets_without_structural_metadata() -> None:
    ...
    }.isdisjoint(ObservationV2.__dataclass_fields__)
...
def test_enemy_activity_does_not_reveal_an_anchor_under_hidden_control() -> None:
    ...
    def active_target(obs: ObservationV2, state: dict[str, Any]) -> AgentAction:
        return AgentAction(ActionKindV2.WRITE, 25, 0xAA)
