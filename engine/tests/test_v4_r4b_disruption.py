...
def test_q1_lost_quota_is_proportional(
    ...
    assert _static_quota_case(shares, disrupted, DisruptedQuotaPolicy.LOST) == expected
...
def test_q2_fair_redistribution_is_explicit(
    ...
    actual = _static_quota_case(
        shares,
        disrupted,
        DisruptedQuotaPolicy.FAIR_REDISTRIBUTION,
    )
...
def test_invalid_declared_quota_is_rejected_instead_of_falling_back() -> None:
    ...
    with pytest.raises(ValueError, match="not a valid DisruptedQuotaPolicy"):
        ProcessMatchController(
            CONFIG,
            [_passive()],
            1,
            disrupted_quota_policy="implicit_fallback",  # type: ignore[arg-type]
        )
...
def _service_process(
    ...
    def logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        ...
        return AgentAction(ActionKind.WRITE, output, 0x22)
...
def _scheduled_attacker(target: int, hit_ticks: set[int]) -> ProcessEntrantSpec:
    def logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        if obs.tick in hit_ticks and state.get("last_hit") != obs.tick:
            state["last_hit"] = obs.tick
        ...
        return AgentAction(ActionKind.WRITE, (target + 30) % 1024, 0xEE)
...
def test_d2_doubles_steady_suppression_efficiency() -> None:
    ...
    controller = ProcessMatchController(
        ...
        [victim, attacker],
        10,
        ProcessModel.MODEL_C_MOVABLE_ANCHOR,
...
def test_dead_entrant_anchor_does_not_accumulate_disruption_metrics() -> None:
    def killer_logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        action_index = state.get("action_index", 0)
        state["action_index"] = action_index + 1
        if obs.tick == 1:
            return AgentAction(ActionKind.WRITE, 341 + action_index, 0xDD)
        return AgentAction(ActionKind.WRITE, 341, 0xEE)
...
def _one_tick_hit_logic(target: int):
    def logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        if obs.tick == 1 and not state.get("hit"):
            state["hit"] = True
            return AgentAction(ActionKind.WRITE, target, 0xFF)
        return AgentAction(ActionKind.WRITE, target + 30, 0xEE)
...
def test_d2_recovery_preserves_state_feedback_and_allows_move_without_duplicate_quota() -> None:
    def victim_logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        observations = state.setdefault("observations", [])
        observations.append(
            {
                "tick": obs.tick,
                "last_kind": obs.last_action_kind,
                "read_result": obs.read_result,
                "counter_before": state.get("counter", 0),
            }
        )
        state["counter"] = state.get("counter", 0) + 1
        if obs.tick == 1:
            return AgentAction(ActionKind.READ, 200)
        if obs.tick == 3 and not state.get("recovery_move"):
            state["recovery_move"] = True
            return AgentAction(ActionKind.MOVE, 1)
        return AgentAction(ActionKind.NOP)
...
def test_d1_monolith_can_trade_movement_for_partial_escape_from_perfect_information_pursuit() -> None:
    def victim_logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        ...
        callback_index = callbacks_by_tick.get(obs.tick, 0)
        callbacks_by_tick[obs.tick] = callback_index + 1
        if callback_index < 2:
            return AgentAction(ActionKind.MOVE, 64)
        state["service_actions"] = state.get("service_actions", 0) + 1
        return AgentAction(ActionKind.WRITE, (obs.position or 0) + 20, 0x22)
    ...
    def attacker_logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        ...
        if distance > (obs.reach or 0):
            state["pursuit_moves"] = state.get("pursuit_moves", 0) + 1
            return AgentAction(ActionKind.MOVE, max(-64, min(64, raw_delta)))
        if state.get("last_hit") != obs.tick:
            state["last_hit"] = obs.tick
            state["disruption_writes"] = state.get("disruption_writes", 0) + 1
            return AgentAction(ActionKind.WRITE, target, 0xFF)
        state["other_actions"] = state.get("other_actions", 0) + 1
...
def _memory_contention_control(ticks: int) -> tuple[ProcessMatchController, ProcessInstance, ProcessInstance]:
    ...
    def attacker_logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        by_tick = state.setdefault("callbacks_by_tick", {})
        callback_index = by_tick.get(obs.tick, 0)
        by_tick[obs.tick] = callback_index + 1
        if callback_index == 7:
            state["contention_writes"] = state.get("contention_writes", 0) + 1
            return AgentAction(ActionKind.WRITE, 920, 0xFF)
        return AgentAction(ActionKind.WRITE, 930, 0xEE)
...
def test_defended_core_attack_is_global_costly_and_non_reversible() -> None:
    def base_logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        index = state.get("core_index", 0)
        state["core_index"] = index + 1
        return AgentAction(ActionKind.WRITE, obs.core_base + index % obs.core_size, 0x11)

    base = ProcessInstance("base", ProcessRole.DEFENDER, 0, 50, 4, base_logic)
    scout = _service_process("scout", 900, 4, 920)
    victim = ProcessEntrantSpec("victim", "victim", [base, scout])

    def core_attacker_logic(obs: ProcessObservation, state: dict[str, Any]) -> AgentAction:
        if obs.position != 981:
            state["moves"] = state.get("moves", 0) + 1
            delta = 981 - (obs.position or 0)
            return AgentAction(ActionKind.MOVE, max(-64, min(64, delta)))
        index = state.get("core_index", 0)
        state["core_index"] = index + 1
        state["core_writes"] = state.get("core_writes", 0) + 1
        return AgentAction(ActionKind.WRITE, index % 8, 0xFF)
