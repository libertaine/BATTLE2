...
# Prefer the Windows launcher with explicit 3.11
$PyCmd  = 'python'
$PyArgs = @()
...
function Ensure-Py311 {
  ...
  try {
    $ver = & $PyCmd @($PyArgs + '-V') 2>$null
  } catch {
    # fall back to current python if launcher isn't available
    $PyCmd  = 'python'
    $PyArgs = @()
    $ver = & $PyCmd -V
  }
  if ($ver -notmatch '3\.11\.') {
    Die "Expected Python 3.11, found: $ver"
  }
...
function New-Or-Reset-Venv {
  ...
  if (-not (Test-Path $VenvDir)) {
    ...
    & $PyCmd @($PyArgs + @('-m','venv', $VenvDir)) | Out-Null
  }
